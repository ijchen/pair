...
use crate::{owner::HasDependent, Owner};
...
impl<O: Owner + ?Sized> Pair<O> {
    ...
    pub fn with_dependent<'a, F: for<'b> FnOnce(&'a <O as HasDependent<'b>>::Dependent) -> U, U>(
        &'a self,
        f: F,
    ) -> U {
        // SAFETY: TODO
    ...
    pub fn with_dependent_mut<
        'a,
        F: for<'b> FnOnce(&'a mut <O as HasDependent<'b>>::Dependent) -> T,
        T,
    >(
        &'a mut self,
        f: F,
